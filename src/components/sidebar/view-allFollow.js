...
export default function ViewAllSuggestedFollow({ profileDocId, username, profileId, loggedInUserdocId, userId }) {
	...
	return (
        <div className='pt-2 pl-3 pb-3 rounded-md justify-between' >
            <div className='flex items-center justify-between'>
                ...
                <div className='mr-3'>
                    {
                        followed === false ? <button type='button' className='text-sm text-white font-bold border rounded-md w-20 h-8 bg-blue-medium focus:outline-none' onClick={handleFollower}>Theo dõi</button>
                                           : loadFollowed ? <button type='button' className=' border rounded-md w-20 h-8 bg-blue-medium focus:outline-none outline-none' >
                                                                 <PulseLoader css={override} loading={followed} color='#fff' size={6}/> 
                                                            </button> 
                                                          : loadUnfollow === false ? <button type='button' onClick={handleUnFollower} className='text-sm font-semibold text-black-bold border-gray-primary border w-28 h-7 rounded-md'>
                                                                                        Đang theo dõi                                                      
                                                                                     </button> 
                                                                                    : loadUnfollow ? 
                                                                                        <button type='button' onClick={handleUnFollower} className='focus:outline-none outline-none border-gray-primary border w-28 h-7 rounded-md'>
