...
export default function Timeline() {
  ...
  return (
    <div className="container col-span-2 p-0 rounded-sm border-gray-primary">
      {!photos ? (
        <>
         {unSkeleton && <Skeleton count={3} width="100%" height={550} className="mb-4" />}
        </>
      ) : (
        photos?.map((content) => (
      ...
      {photos === null || photos.length === 0  ? (
        <>
          ...
          <p className='text-center mt-1 text-gray-base text-sm'><Link className='text-blue-medium font-semibold' to='/profile/Yasuo'>Yasuo</Link> vừa đăng bài viết, hay theo dõi anh ấy để xem những cập nhật mới !</p>
        </>
      ) :''}
    </div>
  );
